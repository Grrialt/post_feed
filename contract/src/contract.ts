...
class GuestBook {
  ...
  add_message({ text, topic, donation }: { text: string, topic: string, donation: string }) {
    ...
    const message: PostedMessage = { premium, sender, time, text, topic, donation };
  ...
  get_messages({ from_index = 0, limit = 100 }: { from_index: number, limit: number }): PostedMessage[] {
